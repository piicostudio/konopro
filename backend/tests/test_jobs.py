...
def test_worker_polling_mode_can_exit_after_empty_polls(tmp_path, monkeypatch, capsys):
    monkeypatch.setenv("KONOPRO_DATABASE_URL", f"sqlite:///{tmp_path / 'worker-loop.db'}")
    monkeypatch.setenv("KONOPRO_STORAGE_ROOT", str(tmp_path / "storage"))
    monkeypatch.setenv("KONOPRO_ENVIRONMENT", "test")

    exit_code = main(["--poll-interval-s", "0", "--max-empty-polls", "1"])

    output = capsys.readouterr().out
    assert exit_code == 0
    assert "Worker polling for queued jobs." in output
    assert "No queued jobs found." in output
